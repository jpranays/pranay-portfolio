  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-6 text-center">
        ...
        >
          ...
          <motion.div variants={item}>
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-mono font-medium bg-green-500/10 text-green-400 border border-green-500/25">
          ...
          <motion.div variants={item} className="h-7 flex items-center justify-center">
            <span className="text-lg sm:text-xl font-mono text-slate-300" aria-live="polite">
              {typed}
              <span
          ...
          <motion.div variants={item} className="flex flex-wrap items-center justify-center gap-2">
            {CREDIBILITY_PILLS.map((pill) => (
              <span
                key={pill.label}
                className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs border border-white/[0.08] bg-white/[0.04] text-slate-400"
              >
                <span className="font-semibold text-slate-200">{pill.label}</span>
                <span>{pill.note}</span>
              </span>
            ))}
          </motion.div>
          ...
          >
            ...
            <span className="text-slate-200 font-medium">Sears India</span> with 3+ years
            building production-grade apps. Creator of npm packages with{" "}
            <span className="text-orange-400 font-medium">25K+ weekly users</span>, and
            open source contributor touching{" "}
            <span className="text-orange-400 font-medium">3.4M+ developers</span> monthly
          ...
          <motion.div variants={item} className="flex items-center gap-5">
            {SOCIAL_LINKS.map(({ href, icon: Icon, label, username }) => (
              <a
                ...
                rel="noopener noreferrer"
                aria-label={label}
                className="group flex items-center gap-1.5 text-slate-600 hover:text-slate-300 transition-colors duration-200"
              >
                <Icon
      ...
      <motion.div
        ...
        aria-hidden="true"
      >
        <span className="text-xs font-mono text-slate-700 tracking-widest uppercase">scroll</span>
        <motion.div
          animate={{ y: [0, 5, 0] }}
