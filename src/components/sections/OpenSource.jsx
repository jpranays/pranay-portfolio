...
const COLOR_MAP = {
	...
	orange:
		"bg-orange-500/10 text-orange-00 dark:text-orange-400 border-orange-500/20",
	purple:
	...
	lightblue:
		"bg-blue-400/10 text-blue-400 dark:text-lightblue-400 border-lightblue-500/20",
};
			...
			<div className="flex items-center justify-between flex-wrap gap-2">
				...
				<div className="flex items-center gap-3">
					...
					<a
						...
						target="_blank"
						rel="noopener noreferrer"
						className="flex items-center gap-1 text-xs font-mono text-orange-500  transition-colors"
					>
					...
					<ul className="space-y-2">
						{topRepos.slice(0, 4).map((repo) => (
							<li key={repo.name}>
                             ...
                             text-slate-400
                            custom-border-color custom-link
                             transition-colors duration-200 group"
								>
									<span className="text-xs font-mono  dark:text-slate-300   truncate">
										{repo.name}
									</span>
									<span className="flex items-center gap-2 flex-shrink-0">
										...
										<span className="flex items-center gap-0.5 text-[10px] font-mono text-yellow-500 dark:text-yellow-400">
...
function buildWeekGrid(contributions) {
	...
	const cells = [...Array(padDays).fill(null), ...contributions];
	...
	return (
		<StaggerItem>
			...
			>
				...
				<p className="text-xs font-mono text-slate-500 dark:text-slate-600 mb-2">
					{item.package}
				</p>
					...
					<div className="flex items-center gap-2">
						{item.npm && (
							<a
								...
								className="text-xs font-mono text-slate-500 dark:text-slate-600 custom-link transition-colors"
							>
								npm{" "}
								<ExternalLink
									className="w-2.5 h-2.5 inline ml-0.5"
									aria-hidden="true"
								/>
							</a>
						)}
			...
			<div className="section-container">
				<AnimatedSection>
					...
					<h2 id="opensource-heading" className="section-title">
						Giving back to the <span className="gradient-text">community</span>
					</h2>
					<p className="section-subtitle">
						I actively contribute to widely-used open-source libraries —
						improving accessibility, fixing bugs, and adding features that
				...
				<div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-4">
					{HEADER_STATS.map((stat, i) => {
						const liveCountTo =
							stat.liveKey === "npm" && npmData
								? Math.round(npmData.total / 1000)
								: stat.liveKey === "oss" && ossData
									? parseFloat((ossData.total / 1_000_000).toFixed(1))
									: stat.countTo;
						return (
							<AnimatedSection key={stat.label} delay={i * 0.1}>
								<div className="glass-card p-5 text-center gradient-border">
									<stat.icon
										className="w-5 h-5 text-orange-400 mx-auto mb-2"
										aria-hidden="true"
									/>
				...
				<AnimatedSection delay={0.3} className="mt-8 text-center">
					<p className="text-sm text-slate-500 dark:text-slate-600 font-mono">
						...
						<a
							...
							target="_blank"
							rel="noopener noreferrer"
							className="text-orange-500 hover:text-orange-400 underline transition-colors ml-1"
						>
							See all on GitHub
						</a>
					</p>
